  ...
  t.test('Central screen is displayed with three tabs', async function (t) {
    t.ok(
      await driver.waitForElementByIosClassChain(
        '**/*[`label BEGINSWITH "Public posts" AND visible == 1`][1]',
        6000,
      ),
    ...
    t.ok(
      await driver.waitForElementByIosClassChain(
        '**/*[`label BEGINSWITH "No messages" AND visible == 1`][1]',
        8000,
      ),
      'I see the Public tab body with no messages',
    );
    t.ok(
      await driver.elementByIosClassChain(
        '**/*[`name BEGINSWITH "Public Tab Button" AND visible == 1`][1]',
      ),
      'I see Public tab button',
    );

    const fab = await driver.elementByIosClassChain(
      '**/*[`name BEGINSWITH "Floating Action Button" AND visible == 1`][1]',
      6000,
    );
