...
exports.create = function(api) {
  ...
  function socialValue(id, key, defaultValue) {
    ...
    return withSync(
      computed([get(id), key, id, yourId, defaultValue], getSocialValue),
    );
  }
...
function getSocialValue(lookup, key, id, yourId, fallback) {
  var result = lookup[key]
    ? getValue(lookup[key][yourId]) ||
      getValue(lookup[key][id]) ||
      highestRank(lookup[key])
    : null;
  if (result != null) {
    return result;
  } else {
