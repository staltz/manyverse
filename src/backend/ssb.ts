...
export function startSSB(isNewIdentity: boolean) {
  ...
  oneTimeFixes().then(() => {
    ...
    // for new users that have just created an identity, only try to
    // decrypt messages created recently to improve onboarding
    // performance
    const startDecryptBox1 = isNewIdentity
